...
#GROWLIDK_DOMAIN
#GROWLIDK_REMOTE_USERNAME
#GROWLIDK_REMOTE_ADDRESS
#GROWLIDK_REMOTE_HOME
...
# copy the keys
cp "$cur_dir/nginx-img/crt/$GROWLIDK_DOMAIN.crt" "$cur_dir/$out_dir/nginx-img/crt/"
cp "$cur_dir/nginx-img/crt/$GROWLIDK_DOMAIN.key" "$cur_dir/$out_dir/nginx-img/crt/"

# path must end in / when uploading a file to a folder
# with duck sftp tunnels to root
duck -i ~/.ssh/id_rsa --upload sftp://$GROWLIDK_REMOTE_USERNAME@$GROWLIDK_REMOTE_ADDRESS$GROWLIDK_REMOTE_HOME "$cur_dir/$out_dir"
